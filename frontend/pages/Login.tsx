...
export default function Login() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const fallbackTimer = setTimeout(() => {
      try {
        ...
        if (!attempted) {
          ...
          console.warn('Login fallback already attempted — clearing loading state');
          setLoading(false);
        }
      } catch (err) {
    ...
    try {
      ...
      const { data: profile, error: profileError } = await supabase.from('profiles').select('role,vendor_id').eq('id', user.id).single();
      console.log('Profile fetch result:', { profile, profileError });
      
      // Navigate based on role
      const targetPath = profile?.role === 'vendor' ? '/vendor' : 
